...
SELECT npi,month,drug_name,bgi,THERAPY_AREA,plan_id,plan_name,plan_type,plan_type_description,
...
from 
p03_phy_plan_TEMP
GROUP BY npi,month,drug_name,bgi,claim_count,patient_count,THERAPY_AREA,plan_id,plan_name,plan_type,plan_type_description,
plan_subtype,payment_type,national_insurer_name,national_type,regional_name,regional_type,organization_name,organization_type,admin_name,admin_type,
SPECIALTY_DESCRIPTION,hospital_id,pg_id,hospital_network_id,pg_network_id,hospital_name,Hospital_Network_Name,PG_NAME,PG_Network_Name,hospital_type,
physician_group_type,340b_id_number,340b_flag
...
p04_claims_child = spark.sql(""" SELECT 
		MONTH,DRUG_NAME,BGI,THERAPY_AREA,PLAN_ID,PLAN_NAME,PLAN_TYPE,PLAN_TYPE_DESCRIPTION,
	PLAN_SUBTYPE,PAYMENT_TYPE,NATIONAL_INSURER_NAME,NATIONAL_TYPE,REGIONAL_NAME,REGIONAL_TYPE,ORGANIZATION_NAME,ORGANIZATION_TYPE,ADMIN_NAME,ADMIN_TYPE,
	SPECIALTY_DESCRIPTION,HOSPITAL_ID,PG_ID,HOSPITAL_NETWORK_ID,PG_NETWORK_ID,HOSPITAL_NAME,HOSPITAL_NETWORK_NAME,PG_NAME,PG_NETWORK_NAME,HOSPITAL_TYPE,
	PHYSICIAN_GROUP_TYPE,340B_ID_NUMBER,340B_FLAG,
	...
	PHYSICIAN_GROUP_TYPE,340B_ID_NUMBER,340B_FLAG
""")
p04_claims_child.write.mode('overwrite').parquet("s3://aws-a0095-use1-00-d-s3b-shrd-shr-input01/Output_Files/RX/"+CURR_TA_S+"/p04_claims_child")
p04_claims_child = spark.read.parquet("s3://aws-a0095-use1-00-d-s3b-shrd-shr-input01/Output_Files/RX/"+CURR_TA_S+"/p04_claims_child")
p04_claims_child.createOrReplaceTempView("p04_claims_child")

spark.sql("SELECT SUM(CLAIM_COUNT) from p04_claims_child") --345038695
...
df =spark.sql("""
	...
	MONTH,DRUG_NAME,BGI,THERAPY_AREA,PLAN_ID,PLAN_NAME,PLAN_TYPE,PLAN_TYPE_DESCRIPTION,PLAN_SUBTYPE,PAYMENT_TYPE,
...
""").write.mode('overwrite').option("path","s3://aws-a0095-use1-00-d-s3b-shrd-shr-input01/Output_Files/RX/"+CURR_TA_S+"/p04_claims_splitting").saveAsTable("hes_stencil.p04_RX_IMM_CHILD_PLAN_DRUG")
...
spark.sql("""SELECT
	...
	FINAL_CHILD,CHILD_COT_TYPE,ADDRESS,CITY,STATE,ZIP_CODE,LHM_ID,LHM_NAME,CBSA,CBSA_NAME,CBSA_TYPE,
	ENTITY_ID,ENTITY_NAME,AHRQ_FLAG,ENTITY_FIRM_TYPE,FINAL_ENTITY_TYPE,REFINED_ENTITY_TYPE,IDN_INTEGRATION_LEVEL,
	sum(CLAIM_COUNT) CLAIM_COUNT, sum(PATIENT_PAY_NORM) PATIENT_PAY_NORM,sum(PLAN_PAY_NORM) PLAN_PAY_NORM from hes_stencil.p04_RX_IMM_CHILD_PLAN_DRUG
	...
	FINAL_CHILD,CHILD_COT_TYPE,ADDRESS,CITY,STATE,ZIP_CODE,LHM_ID,LHM_NAME,CBSA,CBSA_NAME,CBSA_TYPE,
	ENTITY_ID,ENTITY_NAME,AHRQ_FLAG,ENTITY_FIRM_TYPE,FINAL_ENTITY_TYPE,REFINED_ENTITY_TYPE,IDN_INTEGRATION_LEVEL
""").write.mode('overwrite').option("path","s3://aws-a0095-use1-00-d-s3b-shrd-shr-input01/Output_Files/RX/"+CURR_TA_S+"/p04_RX_IMM_CHILD_PLAN_DRUG_2019").saveAsTable("hes_stencil.p04_RX_IMM_CHILD_PLAN_DRUG_2019")
